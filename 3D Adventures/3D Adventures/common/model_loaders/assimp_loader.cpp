...
bool Mesh::InitFromScene(const aiScene* pScene, const string& Filename)
{
	...
	GLint size1;
	ofstream os("cube.aao", ios::binary);
	size1 = Positions.size();
	os.write((const char*)&size1, sizeof(GLint));
	os.write((const char*)&Positions[0], size1 * sizeof(Vector3f));
	size1 = Normals.size();
	os.write((const char*)&size1, sizeof(GLint));
	os.write((const char*)&Normals[0], size1 * sizeof(Vector3f));
	size1 = TexCoords.size();
	os.write((const char*)&size1, sizeof(GLint));
	os.write((const char*)&TexCoords[0], size1 * sizeof(Vector2f));
	size1 = Indices.size();
	os.write((const char*)&size1, sizeof(GLint));
	os.write((const char*)&Indices[0], size1 * sizeof(uint));
	...
	for (int i = 0; i < pScene->mNumMaterials; i++)
	{
		...
		else
		{
			...
			os << "2" << " " << color.r << " " << color.g << " " << color.b << endl;
		}
	}
	os.close();
	...
	for (uint i = 0; i < pScene->mNumMaterials; i++) {
		...
		if (pMaterial->GetTextureCount(aiTextureType_DIFFUSE) > 0) {
			...
			if (pMaterial->GetTexture(aiTextureType_DIFFUSE, 0, &Path, NULL, NULL, NULL, NULL, NULL) == AI_SUCCESS) {
				...
				m_Textures[i] = TextureObject::load_texture(AString::char_to_str(FullPath), GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR_MIPMAP_LINEAR);
		...
		else
		{
			...
			aiColor3D color(0.f, 0.f, 0.f);
			pMaterial->Get(AI_MATKEY_COLOR_DIFFUSE, color);
			printf("%s: %4.2f, %4.2f, %4.2f\n",name.C_Str(), color.r, color.g, color.b);
			m_Textures[i] = TextureObject::create_texture_from_color(color.r, color.g, color.b);
