...
void StaticPalette::ControlPalette(Controller * ctrl)
{
	...
	if (ctrl->GetKey(GLFW_KEY_LEFT_CONTROL))
	{
		...
		if (ctrl->GetKeyOnce(GLFW_KEY_LEFT_CONTROL))
		{
...
void StaticPalette::PlacePalette(Controller * ctrl, std::vector<StaticEntityInfo*>*entity_info, StaticEntity **entity)
{
	...
	if (ctrl->GetKeyOnce(GLFW_KEY_SPACE) && this->visible)
	{
		...
		temp->SetId(this->id);
		temp->SetMatrix(this->GetMatrix(entity[this->id]->GetType(), this->id));

		entity_info[p_info.ind].push_back(temp);



		this->moves.push_back(p_info.ind);
...
void StaticPalette::DeleteLast()
{


	if (this->moves.size() > 0){
		this->entity_info[this->moves[this->moves.size() - 1]].pop_back();
		this->moves.pop_back();
	}
