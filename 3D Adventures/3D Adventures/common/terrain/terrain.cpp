...
void Terrain::Clean()
{
	...
	glDeleteTextures(1, &this->colormap_tex);
	glDeleteTextures(1, &this->texture_array);
	glDeleteTextures(1, &this->normal_texture_array);
...

{
	...
	if (node->GetNodes())
		for (GLuint i = 0; i < 4; i++)
			this->RenderQuad(frustum, node->GetNodes()[i]);
...
void Terrain::LoadColorFiles(char*tile_path, char*texture_set_path, char *normal_set_path, char * texture_attributes_set)
{
	...
	this->colormap_tex = TextureObject::load_texture(tile_path, GL_LINEAR, GL_LINEAR);
	load_texture_set(texture_set_path, this->texture_array, 8.0f);
	load_texture_set(normal_set_path, this->normal_texture_array, 1.0f);
	this->tex_data = new TerrainShader::TextureAttributes[4];
	LoadTextureAttributesSet(texture_attributes_set, this->tex_data);
